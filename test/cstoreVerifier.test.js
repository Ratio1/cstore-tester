...
test('buildExpectedSnapshots returns the expected baseline, delta, and final snapshots', () => {
  ...
  assert.equal(final.k000.version, 'v2');
  assert.equal(final.k100.key, 'k100');
...
test('seedPhase writes the baseline phase and returns a stable digest', async () => {
  ...
  assert.deepEqual(writes, [
    ['cstore:session-1', 'k000', { key: 'k000', version: 'v1' }],
    ['cstore:session-1', 'k001', { key: 'k001', version: 'v1' }],
    ['cstore:session-1', 'k002', { key: 'k002', version: 'v1' }],
    ['cstore:session-1', 'k003', { key: 'k003', version: 'v1' }],
  ]);
});
